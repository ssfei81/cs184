...
{
    matrix newMatrix;
    newMatrix = newMatrix - newMatrix; //zero matrix
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
            for (int k = 0; k < 4; k++) {
                newMatrix.m[i][j] = newMatrix.m[i][j] + (this->m[i][k] * matrix2.m[k][j]) ;
            }
        }
    }
    return newMatrix;
...
vect matrix::operator*(const vect& v) // matrix multiplication with 4x1 vector
	{
        float newX=0.0f, newY=0.0f, newZ=0.0f, newH=0.0f;
            newX = this->m[0][0] * v.getX() + this->m[0][1] * v.getY() + this->m[0][2] * v.getZ() + this->m[0][3] * v.getH();
            newY = this->m[1][0] * v.getX() + this->m[1][1] * v.getY() + this->m[1][2] * v.getZ() + this->m[1][3] * v.getH();
            newZ = this->m[2][0] * v.getX() + this->m[2][1] * v.getY() + this->m[2][2] * v.getZ() + this->m[2][3] * v.getH();
            newH = this->m[3][0] * v.getX() + this->m[3][1] * v.getY() + this->m[3][2] * v.getZ() + this->m[3][3] * v.getH();

        return vect(newX, newY, newZ, newH);
    }

ray matrix::operator*(ray& r) // matrix multiplication with ray
{
    //cheap hack to make sure Point is (x, y, z, 1) and that translations work correctly
    vect newPoint = vect(r.getPoint().getX(), r.getPoint().getY(), r.getPoint().getZ(), 1);
	newPoint = *this * newPoint;

	vect newDir = (*this * r.getDirection()).getUnitVector();

	if(r.getTmax() == FLT_MAX) return ray(newPoint,newDir ,r.getTmin(), r.getTmax());

	vect oldLightPos = r.getPoint() + r.getDirection() * r.getTmax();
	oldLightPos.setH(1);

	vect newLightPos = *this * oldLightPos;
	newLightPos.setH(1);

	float newTmax = (newLightPos-newPoint).getX()/newDir.getX();

	return ray(newPoint,newDir ,r.getTmin(), newTmax);
}
